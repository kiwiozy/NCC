...
def fetch_appointments_from_excel(excel_file: str = "Appointments.xlsx", output_dir: str = "data/reimport", dry_run: bool = False) -> str:
    """
    ...
    if dry_run:
        logger.warning("🔍 DRY RUN MODE - No files will be saved")
        logger.info("")
    ...
    try:
        ...
        if not dry_run:
            ...
            logger.info("")
            logger.warning("🔍 DRY RUN - No file was saved")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dry-run', action='store_true', help='Preview only, do not save files.')
    args = parser.parse_args()
    
    result = fetch_appointments_from_excel(excel_file=args.excel_file, output_dir=args.output_dir, dry_run=args.dry_run)
